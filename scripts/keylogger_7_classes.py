...
print("Press l = left, k, = left-front, f = front, e = right-front, r = right, c = clench, s = swallow, n = none. ESC to stop.")
...
now = datetime.now()
filename = f"7_classes_annotations_{now.strftime('%d-%m-hour-%H-min-%M')}.csv"
...
def on_press(key):
    ...
    try:
        if key.char in ['l', 'k', 'f', 'e', 'r', 'c', 's', 'n']:
